...
class ImageAugmenter:
    ...
    def rotate_polygon(polygon, angle, original_center, new_center, original_dims, new_dims):
        ...
        for x, y in polygon:
            ...
            x_clamped = ImageAugmenter.clamp(x_norm)
            y_clamped = ImageAugmenter.clamp(y_norm)
    ...
    def zoom_out_image_and_polygons(self, image, polygons, min_padding=0.1, max_padding=0.5):
        if not polygons:
        ...
        for polygon in polygons:
            ...
            for x, y in polygon:
                ...
                new_x_clamped = ImageAugmenter.clamp(new_x)
                new_y_clamped = ImageAugmenter.clamp(new_y)
    ...
    def zoom_in_image_and_polygons(self, image, polygons, min_padding=0.1, max_padding=0.8):
        if not polygons:
        ...
        bbox_x_min, bbox_y_min, bbox_x_max, bbox_y_max = ImageAugmenter.calculate_overall_bounding_box(polygons)
    ...
    def crop_image_and_polygons(self, image, polygons, class_ids):
        try:
            img_height, img_width = image.shape[:2]
            ...
            adjusted_polygons, retained_class_ids = self.adjust_polygons_and_class_ids(polygons, class_ids, new_image_box, crop_line_x if crop_orientation == 'vertical' else crop_line_y, keep, crop_orientation)
    ...
    def adjust_polygons_and_class_ids(self, polygons, class_ids, new_image_box, crop_line, keep, orientation):
        adjusted_polygons = []
        ...
        final_polygons = self.adjust_polygons_for_crop(adjusted_polygons, crop_line, keep, orientation)

        return final_polygons, retained_class_ids

    def adjust_polygons_for_crop(self, polygons, crop_line, keep, orientation):
        final_polygons = []
        for polygon in polygons:
    ...
    def pad_image_and_adjust_polygons(self, cropped_image, adjusted_polygons, original_dimensions):
        ...
        for polygon in adjusted_polygons:
            ...
            for x, y in polygon:
                ...
                x_clamped, y_clamped = ImageAugmenter.clamp(x), ImageAugmenter.clamp(y)
                ...
                new_polygon.append((ImageAugmenter.clamp(new_x), ImageAugmenter.clamp(new_y)))
            shifted_polygons.append(new_polygon)

        return padded_image, shifted_polygons

    def overlay_detections_on_coco(self, coco_image, image, detection_polygons, min_scale=0.3, max_scale=1.0):
        adjusted_polygons = []
